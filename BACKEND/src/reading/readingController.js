...
{
    var empolyee = await readingService.getDataFromDBService();
    res.send({ "status": true, "data": empolyee });
}

var createUserControllerFn = async (req, res) => 
{
    var status = await readingService.createUserDBService(req.body);
    if (status) {
...
var updateUserController = async (req, res) => 
{
    ...
    var result = await readingService.updateUserDBService(req.params.id,req.body);
...
{
     console.log(req.params.id);
     var result = await readingService.removeUserDBService(req.params.id);
     if (result) {
