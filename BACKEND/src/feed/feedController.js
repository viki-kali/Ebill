...
{
    var empolyee = await feedService.getDataFromDBService();
    res.send({ "status": true, "data": empolyee });
}

var createUserControllerFn = async (req, res) => 
{
    var status = await feedService.createUserDBService(req.body);
    if (status) {
...
var updateUserController = async (req, res) => 
{
    ...
    var result = await feedService.updateUserDBService(req.params.id,req.body);
...
{
     console.log(req.params.id);
     var result = await feedService.removeUserDBService(req.params.id);
     if (result) {
